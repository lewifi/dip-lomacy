...
// .wrangler/tmp/bundle-6dTKJH/strip-cf-connecting-ip-header.js
...
var GlobalWarDO = class extends DurableObject {
  tendieDips = 12847392;
  // Baseline starting numbers for war vibe
  dimmieDips = 9203118;
  ...
  constructor(ctx, env2) {
    ...
    this.ctx.blockConcurrencyWhile(async () => {
      const storedTendie = await this.ctx.storage.get("tendie_dips");
      const storedDimmie = await this.ctx.storage.get("dimmie_dips");
      if (storedTendie !== void 0)
        this.tendieDips = storedTendie;
      if (storedDimmie !== void 0)
        this.dimmieDips = storedDimmie;
    });
  }
  async fetch(request) {
    ...
    try {
      ...
      if (data.type === "dip" && (data.side === "tendie" || data.side === "dimmie")) {
        ...
        if (!meta)
          return;
        const now = Date.now();
        const interval = meta.lastDipTime > 0 ? now - meta.lastDipTime : 1e3;
        if (interval < 300) {
          return;
        }
        if (interval < 1500) {
        ...
        if (meta.dipIntervals.length >= 10) {
          ...
          const variance = meta.dipIntervals.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / meta.dipIntervals.length;
...
__name(GlobalWarDO, "GlobalWarDO");
...
// .wrangler/tmp/bundle-6dTKJH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6dTKJH/middleware-loader.entry.ts
